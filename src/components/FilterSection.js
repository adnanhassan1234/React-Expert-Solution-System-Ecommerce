...
const FilterSection = () => {
  ...
  const {
    filters: { text, name, color, maxPrice, minPrice, price },
    updateFilterValue,
    all_products,
    clearFilters,
  } = useFilterContext();
  console.log(" FilterSection ~ color:", color);
  ...
  console.log(
    "🚀 ~ file: FilterSection.js:38 ~ FilterSection ~ categoryData:",
    categoryData
  );
  const companyData = getUniqueData(all_products, "company");
  const colorsData = getUniqueData(all_products, "colour");
  console.log(
    "🚀 ~ file: FilterSection.js ~ line 28 ~ FilterSection ~ colorsData",
    colorsData
  );
          ...
          {colorsData.map((currColors, ind) => {
            ...
            return (
              <button
                ...
                className={
                  color === currColors
                    ? "btn btn-info my-1 mx-1 active"
                    : "btn btn-info my-1 mx-1 btnStyle"
                }
                onClick={updateFilterValue}
              >
                {color === currColors ? (
